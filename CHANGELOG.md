...
- 📋 **Komut yardımcısı (cheat-sheet):** Terminal ve Docker panelinde **"📋 Komutlar"** düğmesiyle, kategorize ve **aranabilir** hazır komut listesi açılır (Dosya, Sistem, Servis, Paket, Ağ, **Git**, **Nginx/Web**, Docker… — toplam ~96 komut, iki dilli açıklamalı). Komuta tıklamak onu komut satırına yazar; yer tutucusu olmayan komutların yanındaki **▶** ile **tek tıkla doğrudan çalıştırılır**. Docker panelinde komut sunucu terminalinde çalışır. Yer tutucuları (`<konteyner>`, `<dosya>`) doldurup Enter'a basılır — ezbere gerek yok.
