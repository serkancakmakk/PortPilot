...
async function connectSFTP(cfg, _rawConnect = rawConnectSFTP) {
  const sealed = seal({
  ...
  const state = { conn: null, sftp: null, dead: true, reconnecting: null, closedByUs: false };
  ...
  function ensure() {
    ...
    state.reconnecting = _rawConnect(open(sealed))
      .then(({ conn, sftp }) => {
        state.conn = conn; state.sftp = sftp; state.dead = false;
        watch(conn);
        return sftp;
      })
