...
function platformOf(name) {
  ...
  if (n.endsWith(".appimage") || n.endsWith(".deb")) return { os: "linux", label: "Linux", icon: "🐧" };
...
app.get("/api/downloads", (req, res) => {
  let files = [];
  try { files = fs.readdirSync(DIST_DIR); } catch (_) { return res.json({ available: false, items: [] }); }
  const items = files
    ...
    .map((f) => {
      ...
      return { name: f, size, os: p.os, label: p.label, icon: p.icon, arch: archOf(f),
